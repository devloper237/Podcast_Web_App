document.addEventListener("DOMContentLoaded", () => {
  ...
  function savedSearchesHistory(searchTerm) {
  ...
  //Event listener to load history
  ...
  async function searchPodcast() {
    ...
    if (searchTerm) {
      savedSearchesHistory(searchTerm);
      loadSearchHistory();
    } else {
    ...
    try {
      ...
      if (data.feeds && data.feeds.length > 0) {
        ...
        responseContainer.innerText = "No result found";
      }
    } catch (error) {
  ...
  function createCard(podcast) {
    ...
    pubDate.innerText = `Newest Episode: ${podcast.newestItemPubdate ? formateDate(podcast.newestItemPubdate) : "Not Available"}`;
  ...
  function createEpisodeCard(episode) {
    ...
    pubDate.innerText = `Published: ${episode.datePublished ? formateDate(episode.datePublished) : "Not Available"}`;
